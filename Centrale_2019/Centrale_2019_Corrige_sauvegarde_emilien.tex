...
\begin{remarque}
La question devrait peut être reformulée car la formation de la question : << En considérant $\varepsilon_2$, déterminer (...) >> manque de clarté. 
\end{remarque}
...
$
s_{\text{capteur}}<\dfrac{\SI{75}{\mu m}}{3\sqrt{l_3^2+\dfrac{p^2}{4\pi^2}}}
$.

L'application numérique donne pour résolution maximale afin de respecter le cahier des charge :  $s_{\text{capteur}}= \SI{2,72e-5}{\mu m}$.
...
\begin{remarque}
Il serait sûrement nécessaire de préciser avant la questions que le résultat attendu est en régime permanent.
\end{remarque}
...
\begin{remarque}
En ajoutant l'hypothèse en début de partie II.B.1, l'action de 7 sur 3 est modélisée par un couple. Dans ce cas, le résultat attendu est peut-être 1 inconnue entre 7 et 3.
\end{remarque}
%
%\begin{center}
...
\begin{itemize}
\item Action de la pesanteur sur 3 (\textbf{attention, le schéma cinématique de l'énoncé est donné dans une configuration particulière; la pesanteur crée bien un moment en $B$ suivant  $\vect{z_0}$}) :
...
=\overrightarrow{z_3}\wedge l_3\overrightarrow{y_3}\cdot \left(-m_4\cdot g\cdot \overrightarrow{z_0}\right)=m_4l_3g\overrightarrow{x_3}\cdot \overrightarrow{z_0}
$.

Or,
$\overrightarrow{x_3}=\cos\theta_{32}\overrightarrow{x}_{2,2'}+\sin\theta_{32}\overrightarrow{y}_{2'}$
avec $\cos\theta_{32}\approx 1$ et $\sin\theta_{32}\approx 0$.

 Donc, 
 $\overrightarrow{x_3}\cdot \overrightarrow{z}$ 
 $=\left(\cos\theta_{21}\overrightarrow{x}_{1,0}+\sin\theta_{21}\overrightarrow{y_1}\right)\cdot \overrightarrow{z_0}$  $=\sin\theta_{21}\sin\alpha_1=\dfrac{\sqrt{6}}{4}$.
 On obtient alors, $\vectm{B}{\text{pes}}{4}\cdot \vect{z_4} = l_3 m_4g  \dfrac{\sqrt{6}}{4}$.
...
\begin{remarque}
Il semblerait que ce qui est attendu soit d'utiliser ${C_{73}}=C_{m2}\dfrac{\eta_9}{r_9}$, mais cette relation avait été établie en régime permanent ce qui n'est plus le cas. Continuons avec cette << hypothèse >>.
\end{remarque}
...
\end{itemize}
\begin{remarque}
$J_{eq}$ dépend aussi de $a_3$ et $m_3$, il faudrait ajouter cette précision dans la liste des constantes contenues dans l'expression.
\end{remarque}
...
$C_r(t)= \dfrac{0,78}{0,25} \cdot  53,25  = \SI{166,14}{Nm}$.   


À accélération constante, $\ddot{\theta}_{32} = \dfrac{0,075}{0,25}=\SI{0,3}{rad.s^{-2}}$. On a donc 
$J_{eq}=\dfrac{C_r(t)+A C_{m2}(t)}{\ddot{\theta}_{32}(t)}$
$=\dfrac{C_r(t)-A 51,25}{0,3}$ 
$=\dfrac{166,14  -51,25\dfrac{0,78}{0,25}}{0,3}$
$=\SI{20,8}{kg.m^2}$. 
...
\left\{
\begin{array}{l}
(a) \quad J_{eq}\ddot{\theta}_{32a}=A\cdot C^{a}_{m2}+C_r(t) \\
(b) \quad 0=A\cdot C^{b}_{m2}+C_r(t)\\
(c) \quad -J_{eq}\ddot{\theta}_{32a}=A\cdot C^{c}_{m2}+C_r(t)\\
\end{array}
\right.
$.
...
On obtient alors, $
J_{eq}=\dfrac{A\cdot \left(C^{a}_{m2}-C^{c}_{m2}\right)}{2\ddot{\theta}_{32a}} \approx \SI{19,76}{kg.m^2}$.
...
\begin{remarque}
On peut hésiter entre prendre la structure de la partie III.A (Méthode A) ou donner un $K(p)$ équivalent sous la forme d'un correcteur P.D. (Méthode B).
\end{remarque}
...
\item \textbf{Rapidité : } le temps de réponse à $5\%$ est la durée entre l'instant de l'échelon et l'instant au delà duquel la position est comprise entre $\pm 5\%$ de la valeur finale et qui correspond ici à la durée au delà de laquelle la position est comprise entre $\SI{0,57}{\mu m}$ et $\SI{0,63}{\mu m}$. Sur la figure 18, on relève approximativement $\SI{0,15}{s}<\SI{0,2}{s}$ comme l'exige l'exigence 1.2.2.  
\end{itemize}
